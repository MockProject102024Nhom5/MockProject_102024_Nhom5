...
const ResidentList: React.FC = () => {
  ...
  const handleResidentClick = (resident: Resident) => {
    ...
    setPassword(''); // Không hiện password
  };

  const handleUpdate = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      await axios.put(`https://671200354eca2acdb5f6890f.mockapi.io/api/residents/${residentId}`, {
        fullName,
        role,
        email,
        phoneNumber,
        password, // Gửi password nếu có thay đổi
      });
      fetchResidents(); // Cập nhật lại danh sách sau khi sửa
    } catch (error) {
  ...
  return (
    <div className="container mx-auto p-4 bg-white rounded-lg shadow-md">
      ...
      <div className="form-container grid grid-cols-2 gap-4 mb-4">
      ...
      <div className="search-container mb-4 flex justify-center items-center">
        <input
          type="text"
          id="searchQuery"
      ...
      <h2 className="text-xl font-semibold mb-4">List Apartment</h2>
        ...
        <table className="min-w-full border-collapse border border-gray-300">
          ...
          <tbody id="residentTableBody">
            {residents.map((resident) => (
              <tr key={resident.residentId} className="text-center hover:bg-gray-100 cursor-pointer" onClick={() => handleResidentClick(resident)}>
